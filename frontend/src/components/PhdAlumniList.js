...
import {listPhdAlumnis} from '../actions/AlumniActions'
...
export default function PhdAlumniList({header}) {
  ...
  const phd_alumnis_s = useSelector (state => state.listPhdAlumnis)
  const {error, loading, phd_alumni} = phd_alumnis_s

  useEffect (()=>{
    dispatch(listPhdAlumnis());
  }, [])
