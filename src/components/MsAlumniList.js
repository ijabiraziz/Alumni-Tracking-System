...
import { listMsAlumnis} from '../actions/AlumniActions'
...
export default function MsAlumniList({header}) {
  ...
  const ms_alumnis_s = useSelector (state => state.listMsAlumnis)
  const {error, loading, ms_alumni} = ms_alumnis_s

  useEffect (()=>{
    dispatch(listMsAlumnis());
  }, [])
